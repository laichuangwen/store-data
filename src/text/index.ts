...
type ResolvedConfig = typeof defaultConfig & CanvasTextConfig
...
export class Text {
  ...
  constructor(
    ...
    this.pad = normalizePadding(this.config.padding)
    this.box = this.createContentBox()
  }

  /** 在指定矩形区域内绘制多行文本 */
  draw(): DrawTextResult {
    if (!this.isValid()) return EMPTY_RESULT

    const { config, pad } = this
    ...
    if (config.autoRowHeight) {
      ...
      if (neededBoxHeight > config.height) {
        config.onHeight?.(neededBoxHeight)
      }
    }

    this.paintSelection(layout)
    this.paintLines(lines, textAnchor)
  ...
  /** 布局扁平文本 */
  getFlatText(): string {
    return this.ensureLayout().flatText
  }

  private ensureLayout(): TextLayout {
    ...
    const charHeight = this.measureCharHeight(style)
    const prepared = this.prepareLines(charHeight)
    const drawnHeight = linesHeight(prepared.lines.length, charHeight)
    const textAnchor = this.applyHorizontalAlign()
  ...
  private paintSelection(layout: TextLayout): void {
    const range = this.normalizedSelection(layout.flatText.length)
    if (!range) return
    ...
    ctx.fillStyle = config.selectionColor || DEFAULT_SELECTION_COLOR
  ...
  private buildFontStyle(): string {
    const { fontStyle, fontVariant, fontWeight, fontSize, font } = this.config
    return `${fontStyle} ${fontVariant} ${fontWeight} ${fontSize}px ${font}`.trim()
  }

  private measureCharHeight(style: string): number {
    const { lineHeight, fontSize } = this.config
    if (lineHeight != null) return lineHeight
    return Math.max(
      getTextHeight({ ctx: this.ctx, text: 'Mg', style, fallback: fontSize }),
      fontSize
    )
  }

  private prepareLines(charHeight: number): {
    lines: string[]
    clamped: boolean
    neededHeight: number
  } {
    const { ctx, text, config, box } = this
    ...
    if (!config.autoRowHeight && neededHeight > contentHeight) {
      ;({ lines } = clampLines(
        ctx,
        lines,
  ...
  private applyHorizontalAlign(): number {
    const { ctx, config, box } = this
  ...
  private resolveVerticalAlign(drawnHeight: number): {
    ...
    const { verticalAlign } = this.config
  ...
  private paintLines(lines: LayoutLine[], textAnchor: number): void {
    const { ctx, config } = this
